...
const TABS = [
  { path: '/',        en: 'Name Generator', ja: '名前を作る' },
  { path: '/ranking', en: 'Rankings',       ja: '人気ランキング' },
  { path: '/zodiac',  en: '🐯 Zodiac',      ja: '🐯 十二支' },
  { path: '/fortune', en: '🔮 Fortune',     ja: '🔮 運勢' },
]
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-brand-50 to-gray-50">
        ...
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3">
          ...
          <div className="relative">
            ...
            >
              ...
              <svg className={`h-3.5 w-3.5 text-gray-400 transition-transform ${showLangMenu ? 'rotate-180' : ''}`}
          ...
          <nav className="flex overflow-x-auto scrollbar-none">
            {TABS.map(tab => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `relative shrink-0 px-4 py-2.5 text-sm font-semibold transition whitespace-nowrap
      ...
      {/* ── 메인 레이아웃 (PC 중앙 정렬) ── */}
      <div className="mx-auto max-w-4xl px-4 py-6 lg:flex lg:justify-center lg:gap-6">
        <main className="w-full max-w-2xl">
          <Routes>
            <Route path="/"        element={<NameGenerator lang={lang} />} />
            <Route path="/ranking" element={<NameRanking   lang={lang} />} />
            <Route path="/zodiac"  element={<ZodiacPage    lang={lang} />} />
            <Route path="/fortune" element={<FortunePage   lang={lang} />} />
          </Routes>
        </main>
      ...
      <footer className="mt-8 border-t border-gray-200 bg-white py-6 text-center text-xs text-gray-400">
        ...
        <div className="mt-1 flex justify-center gap-4">
          <button onClick={() => setShowPrivacy(true)} className="underline-offset-2 hover:underline">
            {t(lang, 'footer.privacy')}
          </button>
          <a href={`mailto:${SITE_CONFIG.CONTACT_EMAIL}`} className="underline-offset-2 hover:underline">
